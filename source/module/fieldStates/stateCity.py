...
class StateCity(BaseFieldState):
    ...
    def __init__(self, stateStack):
        ...
        # 座標は"0101U"のようにX座標とY座標を2桁にした値と方向の値を結合し、"U"(update用)か"D"(draw用)を付与したものとする
        self.event = {
    ...
    def draw_gate(self):
        ...
        for _x in range(12, 66, 4):
            pyxel.rect(_x + self.OFFSET_X, 12 + self.OFFSET_Y, 2, 54, pyxel.COLOR_BLACK)
        pyxel.rect(32 + self.OFFSET_X, 29 + self.OFFSET_Y, 18, 9, pyxel.COLOR_YELLOW)
        PyxelUtil.text(33 + self.OFFSET_X, 30 + self.OFFSET_Y, "*GATE", pyxel.COLOR_BLACK)

    def draw_shieldshop(self):
        '''
        盾屋の前に立った時の表示
        '''
        PyxelUtil.text(30 + self.OFFSET_X, 22 + self.OFFSET_Y, "*SHIELD", pyxel.COLOR_BLACK)

    def draw_armorshop(self):
        '''
        鎧屋の前に立った時の表示
        '''
        PyxelUtil.text(32 + self.OFFSET_X, 22 + self.OFFSET_Y, "*ARMOR", pyxel.COLOR_BLACK)

    def draw_weaponshop(self):
        '''
        武器屋の前に立った時の表示
        '''
        PyxelUtil.text(30 + self.OFFSET_X, 22 + self.OFFSET_Y, "*WEAPON", pyxel.COLOR_BLACK)

    def draw_helmetshop(self):
        '''
        兜屋の前に立った時の表示
        '''
        PyxelUtil.text(30 + self.OFFSET_X, 22 + self.OFFSET_Y, "*HELMET", pyxel.COLOR_BLACK)

    def draw_barbar(self):
        '''
        床屋の前に立った時の表示
        '''
        PyxelUtil.text(30 + self.OFFSET_X, 22 + self.OFFSET_Y, "*BARBAR", pyxel.COLOR_BLACK)
    ...
    def draw_inn(self):
        ...
        宿屋の前に立った時の表示
        '''
        PyxelUtil.text(22 + self.OFFSET_X, 22 + self.OFFSET_Y, "*UTSURO INN", pyxel.COLOR_BLACK)

    def draw_physicker(self):
        '''
        研究所の前に立った時の表示
        '''
        PyxelUtil.text(24 + self.OFFSET_X, 22 + self.OFFSET_Y, "*PHYSICKER", pyxel.COLOR_BLACK)

    def draw_physicker_exit(self):
        '''
        研究所の出口の前に立った時の表示
        '''
        PyxelUtil.text(34 + self.OFFSET_X, 22 + self.OFFSET_Y, "*EXIT", pyxel.COLOR_BLACK)

    def draw_drugs(self):
        '''
        薬屋の前に立った時の表示
        '''
        PyxelUtil.text(32 + self.OFFSET_X, 22 + self.OFFSET_Y, "*DRUGS", pyxel.COLOR_BLACK)

    def draw_surgery(self):
        '''
        身体検査の前に立った時の表示
        '''
        PyxelUtil.text(28 + self.OFFSET_X, 22 + self.OFFSET_Y, "*SURGERY", pyxel.COLOR_BLACK)

    def draw_examinations(self):
        '''
        緊急治療の前に立った時の表示
        '''
        PyxelUtil.text(18 + self.OFFSET_X, 22 + self.OFFSET_Y, "*EXAMINATIONS", pyxel.COLOR_BLACK)
    ...
    def draw_jail(self):
        ...
        牢屋の前に立った時の表示
        '''
        PyxelUtil.text(32 + self.OFFSET_X, 22 + self.OFFSET_Y, "*JAIL", pyxel.COLOR_BLACK)
        
    def draw_secretmessage(self):
        '''
        隠しメッセージ
        '''
        PyxelUtil.text(136, 112, ["I", "RO", " ", "I", "LTU", "KA", "I", " ", "TU", "D", "TU", "*..."], pyxel.COLOR_WHITE)
    ...
    def draw_temple(self):
        ...
        寺院への看板の表示
        '''
        PyxelUtil.text(30 + self.OFFSET_X, 22 + self.OFFSET_Y, "*TEMPLE", pyxel.COLOR_BLACK)
